...
pub enum ErrorKind {
    ...
    /// Client does not support authentication protocol requested by server; consider upgrading MariaDB backend
    ...
    /// @@SESSION.GTID_NEXT cannot be changed by a backend that owns a GTID. The backend owns %s. Ownership is released on COMMIT or ROLLBACK.
    ...
    /// Your password has expired. To log in you must change it using a backend that supports expired passwords.
