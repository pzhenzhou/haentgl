...
use crate::cp::prost::control_plane::control_plane_service_server::ControlPlaneServiceServer;
...
pub mod control_plane {
    include!("codegen/control_plane/control_plane.rs");
}
...
pub async fn start_cp_target_reporter(
    ...
    let cp_srv_impl = cp_target::ControlPlaneServiceImpl::new(active_users);
    let cp_addr_socket = cp_addr.parse().unwrap();
    tokio::task::spawn(async move {
        tonic::transport::Server::builder()
            .timeout(Duration::from_secs(5))
            .add_service(ControlPlaneServiceServer::new(cp_srv_impl))
