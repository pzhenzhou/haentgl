use crate::cp::prost::control_plane::{EloqCluster, UserCom};
...
fn user_com_hash(user_com: &UserCom) -> u64 {
    ...
    hasher.write_str(&user_ref.az);
    hasher.write_str(&user_ref.cluster);
...
pub struct UserActivityWindow {
    ...
    size: Arc<AtomicU64>,
    count: Arc<AtomicU64>,
    buf: mpsc::UnboundedSender<(EloqCluster, String, u8, u64)>,
}
...
mod tests {
    ...
    pub async fn test_active_user_com() {
        ...
        for i in 0..max_user {
            ...
            let az = az_dic[i % 3];
            let cluster = control_plane::EloqCluster {
